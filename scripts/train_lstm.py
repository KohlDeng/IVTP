...
def main():
    ...
    parser.add_argument('--epochs', type=int, default=80, metavar='E', help='no. of epochs to run (default: 10)')
    ...
    parser.add_argument('--weight',type=str,default="checkpoint/lstm_weight.pth.tar",help="weight file name(default:checkpoint/mlp_weight.pth.tar)")
    
    hyperparams = parser.parse_args()
    transform = transforms.Compose([transforms.ToTensor()])
    
    trainset = HjdriveSet(sample_dir,ifscaler=True,scaler_file='config/hjdrive_30000_mminfo.npy')
    # trainset = HjdriveSet(sample_dir)
    ...
    model = IVLSTM(in_feature,in_steps,out_feature,out_steps,num_layers,device=dev).to(dev)
    print(model)
...
def train(model,trainloader,hyperparams,dev,path=None):
    ...
    for epoch in range(1+run_epoch,epochs+1+run_epoch):  
        ...
        if(epoch%50==0):    
            ...
            weight_name = "checkpoint/lstm_weight_epoch_"+str(epoch)+".pth.tar"
    ...
    plt.ylabel("Loss")
    plt.legend()
    figure_dir = os.path.join(prj_dir,"checkpoint/lstm_train_loss.jpg")
